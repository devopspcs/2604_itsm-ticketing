...
func (r *ticketRepository) List(ctx context.Context, filter repository.TicketFilter) (*repository.PaginatedTickets, error) {
	...
	if filter.AssignedTeamID != nil {
		where += fmt.Sprintf(" AND assigned_team_id=$%d", i)
		args = append(args, *filter.AssignedTeamID)
		i++
	}
	if len(filter.AssignedTeamIDsOrUnassigned) > 0 {
		placeholders := ""
		for idx, tid := range filter.AssignedTeamIDsOrUnassigned {
			if idx > 0 {
				placeholders += ","
			}
		...
		where += fmt.Sprintf(" AND (assigned_team_id IN (%s) OR assigned_team_id IS NULL)", placeholders)
	}
...
func (r *ticketRepository) Update(ctx context.Context, ticket *entity.Ticket) error {
	query := `UPDATE tickets SET title=$1, description=$2, category=$3, priority=$4, status=$5, assigned_to=$6, assigned_team_id=$7, updated_at=$8, resolved_at=$9 WHERE id=$10`
	_, err := r.db.Exec(ctx, query, ticket.Title, ticket.Description, ticket.Category, ticket.Priority, ticket.Status, ticket.AssignedTo, ticket.AssignedTeamID, time.Now().UTC(), ticket.ResolvedAt, ticket.ID)
	return err
}
