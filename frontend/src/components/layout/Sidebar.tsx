  ...
  return (
    <aside className="h-screen w-64 fixed left-0 top-0 pt-16 bg-slate-50 flex flex-col gap-2 p-4 z-40 hidden md:flex">
      <div className="mb-6 px-2">
        <h2 className="text-lg font-black text-blue-900 font-headline">Service Console</h2>
      ...
      <nav className="flex flex-col gap-1 flex-1">
        {visible.map((item) => (
          <NavLink
            ...
            className={({ isActive }) =>
              isActive
                ? 'flex items-center gap-3 px-3 py-2.5 text-blue-900 font-bold bg-white rounded-lg text-sm shadow-sm'
                : 'flex items-center gap-3 px-3 py-2.5 text-slate-600 hover:text-blue-700 hover:translate-x-1 transition-all text-sm font-medium'
            }
          >
            {({ isActive }) => (
              <>
